  ...
  return {
    ...
    visitor: {
      ImportDeclaration(path) {
        if (
					t.isStringLiteral(path.node.source) && (validImportSources.includes(path.node.source.value))
        ) {
					const { specifiers } = path.node
					const asyncComponentImport = specifiers.filter(specifier => t.isImportDefaultSpecifier(specifier) || t.isImportSpecifier(specifier) && specifier.imported.name === "asyncComponent")
...
const importVisitor = {
  Import(path) {
    ...
    this.parentPath.insertBefore(
      this.t.objectProperty(this.t.stringLiteral("chunkName"), this.t.stringLiteral(chunkName))
    );
  }
};
